      ...
      <div className="flex flex-col">
        <div className="flex flex-row">
          {successMessage ? (
            ...
            <>
              <input type="email" placeholder={errorMessage || placeholder} onChange={(e) => setEmail(e.target.value)} value={email} className={"w-3/5 p-3 rounded-l-lg sm:w-2/3 text-teal-200"} />
              <button type="button" className="w-2/5 p-3 font-semibold rounded-r-lg sm:w-1/3 bg-orange-400 text-slate-50" onClick={handleSubmit}>
                {text}
              </button>
            </>
          )}
